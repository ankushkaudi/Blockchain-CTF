...
app.post('/submit', upload.single('file'), (req, res) => {
  const { challengeName, challengeDescription, points } = req.body;
  const filePath = req.file.path;

  const sql = 'INSERT INTO challenges (name, description, file_path, points) VALUES (?, ?, ?, ?)';
  db.query(sql, [challengeName, challengeDescription, filePath, points], (err, result) => {
    if (err) {
      console.error(err);
